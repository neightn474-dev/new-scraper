...
export function buildCompaniesFromJobs(jobs) {
  ...
  for (const job of jobs) {
    ...
    if (!companies.has(companyKey)) {
      companies.set(companyKey, {
        ...
        business_model: makeEvidence(
          inferBusinessModel(job),
          job.job_url,
          inferBusinessModel(job) === 'not found' ? 'none' : 'low',
          inferBusinessModel(job) !== 'not found'
        ),
        ...
        industry: makeEvidence(
          inferIndustry(job),
          job.job_url,
          inferIndustry(job) === 'not found' ? 'none' : 'low',
          inferIndustry(job) !== 'not found'
        ),
        location: makeEvidence(
          job.location || 'not found',
          job.job_url,
          job.location && job.location !== 'not found' ? 'low' : 'none',
          true
        ),
  ...
  for (const company of companies.values()) {
    company.hiring_intent_summary = makeEvidence(
      summarizeHiringIntent(company, jobs),
      'derived from current job mix',
      'medium',
      true
    );
  }
...
export function attachCompanyDataToJobs(jobs, companies) {
  ...
  for (const company of companies) {
    for (const jobId of company.job_ids) {
      byJobId.set(jobId, company);
    }
  }
...
function summarizeHiringIntent(company, allJobs) {
  ...
  if (!top || top[0] === 'not found') {
    return `Company has ${jobs.length} active public hiring signal(s); direction requires additional enrichment.`;
  }
...
function firstCareerUrl(job) {
  if (job.source === 'ashby') {
    return job.source_url?.replace('/posting-api/job-board/', '/').replace(/\?.*$/, '') || job.job_url;
  }

  if (job.source === 'lever') {
    return job.source_url
      ?.replace('https://api.lever.co/v0/postings/', 'https://jobs.lever.co/')
      .replace(/\?.*$/, '') || job.job_url;
  }

  if (job.source === 'greenhouse') {
    return job.source_url
      ?.replace('https://boards-api.greenhouse.io/v1/boards/', 'https://boards.greenhouse.io/')
      .replace('/jobs?content=true', '') || job.job_url;
  }
...
function employeeRangeToEvidence(range) {
  if (!range || range.label === 'not found') {
    return makeEvidence('not found', null, 'none', false);
  }
