...
export async function resolveCompanyWebsite(company, jobs, http, options = {}) {
  ...
  if (discovery.fetchCareerPageLinks !== false) {
    ...
    for (const pageUrl of pagesToFetch) {
      try {
        if (!(await http.isRobotsAllowed(pageUrl, { enabled: options.respectRobotsTxt !== false }))) {
          candidates.push({ url: null, source: 'robots_txt_disallowed', sourceUrl: pageUrl, score: 0, reasons: ['robots_txt_disallowed'] });
          continue;
        }
