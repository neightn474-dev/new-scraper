...
import { attachCompanyDataToJobs, enrichCompanies, filterCompaniesByEmployeeRange, buildCompaniesFromJobs } from './enrichment/company.js';
...
export async function runScraper(input) {
  ...
  let companies = await enrichCompanies(buildCompaniesFromJobs(jobs), http, { ...input, jobs });
...
function normalizeInput(input) {
  return {
    ...
    maxCompaniesToEnrich: Number(input.maxCompaniesToEnrich) > 0 ? Number(input.maxCompaniesToEnrich) : DEFAULT_INPUT.maxCompaniesToEnrich,
    employeeRangeFilter: { ...DEFAULT_INPUT.employeeRangeFilter, ...(input.employeeRangeFilter || {}) },
    websiteDiscovery: { ...DEFAULT_INPUT.websiteDiscovery, ...(input.websiteDiscovery || {}) },
    employeeRangeDiscovery: { ...DEFAULT_INPUT.employeeRangeDiscovery, ...(input.employeeRangeDiscovery || {}) },
  };
}
